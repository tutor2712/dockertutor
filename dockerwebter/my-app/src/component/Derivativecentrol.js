...
window.$fx = ""
// Secant
class Derivativecentrol extends Component {
  constructor(props)
  {
      super(props);
      this.state = { users: "",
  ...
  render() {
   ...
   <div><h3>O(h^2) click1  O(h^4) click2: </h3></div>
   <h3><input onChange={(event)=>{window.$oh = event.target.value;console.log(window.$oh)}}/></h3>
   <button onClick = {this.derivativecentrol}> click </button>
   <br/>
      <h3>database </h3>
      <button onClick = {this.derivativecentrol}> fx : {window.$fx} <br/> x : {window.$x} <br/> h : {window.$h}<br/> d : {window.$d}<br/> oh : {window.$oh}</button>
   <div>
       <table> 
            {this.createTable()}
       </table>
...
export default Derivativecentrol;