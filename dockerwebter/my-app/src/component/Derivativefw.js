...
window.$fx = ""
// Secant
class Derivativefw extends Component {
  constructor(props)
  {
      super(props);
      this.state = { users: "",
    ...
    var ans = 0
        ...
        if(oh == 1){
            if(d == 1){
                ...
                ans = (funtion(fx,x+(3*h)) - 3*funtion(fx,x+(2*h)) + 3*funtion(fx,x+h) - funtion(fx,x))/pow(h,3)
                console.log("ans : " + ans)
            }
            else if(d == 4){
                ans = (funtion(fx,x+(4*h)) - 4*funtion(fx,x+(3*h)) + 6*funtion(fx,x+(2*h)) - 4*funtion(fx,x+h) + funtion(fx,x))/pow(h,4)
                console.log("ans : " + ans)
            }
        }
        else if(oh == 2){
            if(d == 1){
                ans = (-funtion(fx,x+(2*h)) + 4*funtion(fx,x+h) - 3*funtion(fx,x))/(2*h)
            }
            else if(d == 2){
                ans = (-funtion(fx,x+(3*h)) + 4*funtion(fx,x+(2*h)) - 5*funtion(fx,x+h) + 2*funtion(fx,x))/pow(h,2)
            }
            else if(d == 3){
  ...
  render() {
   ...
   <button onClick = {this.Forward}> click </button>
   <br/>
      <h3>database </h3>
      <button onClick = {this.Forward}> fx : {window.$fx} <br/> x : {window.$x} <br/> h : {window.$h}<br/> d : {window.$d}<br/> oh : {window.$oh}</button>
   <div>
       <table> 
            {this.createTable()}
       </table>
